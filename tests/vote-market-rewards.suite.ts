...
import {
    ...
    GAUGE_PROGRAM_ID,
    GAUGEMEISTER,
    LOCKED_VOTER_PROGRAM_ID, PROGRAM_ID, REWARD_PROGRAM_ID,
} from "./constants";
...
export function rewardsSuite(cfg: RunCfg) {
    describe(`[${cfg.rewardStyle}] rewards (${cfg.rewardStyle})`, () => {
        ...
        const program = new Program(IDL, PROGRAM_ID ,provider) as Program<VoteMarket>;
            ...
            it("Vote sellers can withdraw vote payment", async () => {
                ...
                if (cfg.rewardStyle === "rewardAccumulator") {
                    //create reward accumulator config
                    const hash = crypto.createHash("sha256").update("votex").digest();
                    const namespace = hash.subarray(0, 8);
                    const [rewardAccumulatorConfig, _] = web3.PublicKey.findProgramAddressSync(
                        [Buffer.from("reward-accumulator-config"), config.publicKey.toBuffer()],
                        program.programId
                    );
                     await program.methods.updateRewardAccumulatorConfig(
                        REWARD_PROGRAM_ID,
                        namespace).accounts({
                ...
                if(cfg.rewardStyle === "rewardAccumulator") {
                    const [rewardAccumulatorConfig] = web3.PublicKey.findProgramAddressSync(
                        [
                            Buffer.from("reward-accumulator-config"),
                            config.publicKey.toBuffer()],
                        program.programId);
                    const hash = crypto.createHash("sha256").update("votex").digest();
                    const namespace = hash.subarray(0, 8);

                    const [sellerRewardPda] = web3.PublicKey.findProgramAddressSync(
                        [
                        ...
                        REWARD_PROGRAM_ID);
                    const sellerRewardAta = getAssociatedTokenAddressSync(mint, sellerRewardPda, true);
                    const createAccountIx = await createAssociatedTokenAccountInstruction(payer.publicKey,
                        sellerRewardAta,
                        sellerRewardPda,
                        mint);
                    ...
                    const tx = await program.methods.claimToRewardAccumulator(gaugeMeisterData.currentRewardsEpoch + 1)
                        .accounts({
                            scriptAuthority: nonSellerPayer.publicKey,
                            seller: program.provider.publicKey,
                            sellerTokenAccount: sellerRewardAta,
                            tokenVault,
                            treasury,
                            admin: newAdmin.publicKey,
                            mint,
